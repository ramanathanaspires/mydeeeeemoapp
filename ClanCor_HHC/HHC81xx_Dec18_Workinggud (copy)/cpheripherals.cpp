...
bool CPheripherals::setBacklight(int nValue)
{
    if ( nValue < 0 || nValue > 5) {
        ...
        bool bResponse;
        pauseThread();
        bResponse = sendData(const_cPHERIPHERAL_IO_CONTROL,const_cPACKETID_TFT_BACKLIGHT, "T" + gAlignNumber(nValue,2),3000);
        if ( m_bThreadPause ) {
            startThread();
        }
        return bResponse;
    ...
    */
    ...
    if ( bResponse ) {
        ...
        do {
            ...
            bRePrint = false;
            bResponse = sendData(const_cPHERIPHERAL_PRINTER, const_cPACKETID_PRINT_IMAGE, QString(const_cPRINT_PHOTO) + gAlignNumber(nWidth, 2) , 5000);
            if ( bResponse ) {
                if ( ! m_bPrinterReady )
                {
                    if(gbEnableMessageBoxPrinter)
...
//        qDebug()<<"\r"<<"transmitted:" + bytearrayPrintData;
