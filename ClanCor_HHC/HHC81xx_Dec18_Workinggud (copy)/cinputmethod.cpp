    ...
    m_nScreenLockTime(0), m_nIdleTime(0),m_nStandbyTime(0)
{
    ...
    getDateTime();
//    ggsmModuleSimCom->getGPSPosition();
    //    autoPowerControl();
    m_nErrorCountWifi = 0;
}
                          ...
                          bool bIsPress, bool bAutoRepeat)
{
    ...
    if ( m_bRunning ) {
        ...
        if ( eventFilter(nKeycode) ) {
            ...
            if ( m_bActive ) {
                /*  mobile keypad is active -- accept all key input if numeric mode active*/
                if(gekeypadModeCurrent == eKeypadMode_NUMERIC)
                {
                    /* check if retrieved key code is exclam (!), to show symbol dialog */
                    if(nKeycode != Qt::Key_Exclam)          /* Key_At changed to Key_Exclam, due to Key_At added into char rotation on key 1*/
                    {
                        return false;
                    }
                }
            ...
            if(!(( nKeycode >= Qt::Key_0 && nKeycode <= Qt::Key_Z) || nKeycode == Qt::Key_Shift
                 || nKeycode == Qt::Key_Space || nKeycode == Qt::Key_At|| nKeycode == Qt::Key_Period
...
#endif
                        m_timerKeypad.start();  /* Start Key rotation timer */
...
{
//     qDebug()<<"\r Shutdown given by user ";
    /* shutdown the device */
...
//    qDebug()<<"\r PIC shutdown given ";
//    qDebug()<<"\r Shutdown OS now";
...
//        qDebug()<<"Reset keyboard";
        gserverObj->closeKeyboard();
//        qDebug()<<"Reset keyboard 1";
        gserverObj->openKeyboard();
//        qDebug()<<"Reset keyboard 2";
        gserverObj->refresh();
//        qDebug()<<"Reset keyboard 3";
//        gserialportDebug->close();
//        qDebug()<<"Reset keyboard 4";
...
void CInputMethod::wakeup(bool bFlagStandby)
{
    ...
    gUpdateScreen();
//    qDebug()<<"\rwakeup from idle gsm"<<ggsmModuleSimCom->isInitiated()<<gbGprsConnected;
    ggsmModuleSimCom->enableGprs();//For testingNov12013
    if ( ggsmModuleSimCom->isInitiated() && ( ! gbGprsConnected ) )
    {
        if ( bFlagStandby )
        {
//            qDebug()<<"\r wakeup bFlagStandby.."<<"in flag true";
        ...
        {
//            qDebug()<<"\r wakeup bFlagStandby..>>"<<"in flag false";
//            ggsmModuleSimCom->initModule();//For testing on OCt29_2013
            ggsmModuleSimCom->startThread();
        }
    }
    waitForKeyboard();
...
//          qDebug()<<"\r wakeup from idle mode in input method..>>";
        gWriteDataToComport("Wakeup from Idle Mode");

        //For testing have implement below on Now 31 2013
//        gpheripheralsPIC->enableModule(CPheripherals::emodule_GPRS,true);
//        ggsmModuleSimCom->enableGprs();
//        sleep(3);
//        ggsmModuleSimCom->initModule();
//        ggsmModuleSimCom->startThread();
        //For testing have implement above on Now 31 2013
        emit wakeupFromIdleMode();
    }
}

//void CInputMethod::on_timerWifiSearch_timeout()
//{
//    /* search wifi device if not found */
//    if( ! gnetworkActive->isWifiFound() ) {
//        gnetworkActive->searchWifiDevice();
//        if ( m_nErrorCountWifi == 4 ) {
//            /* try 4 times in first time only */
//            m_timerWifiSearch.stop();
//        }
//        else {
//            m_nErrorCountWifi ++;
//        }
