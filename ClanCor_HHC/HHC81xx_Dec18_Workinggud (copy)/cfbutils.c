...
int open_framebuffer(void)
{
    ...
    /* retrieve environment variable value for TSLIB_FBDEVICE */
    if ((fbdevice = getenv ("TSLIB_FBDEVICE")) == NULL)
        fbdevice = defaultfbdevice;

    /* retrieve environment variable value for TSLIB_CONSOLEDEVICE */
    ...
    printf("fbdev %s opened",fbdevice);

    /* system call to get display screen info  */
    ...
    /* system call to get display screen resolution info  */
    ...
    /* allocate memeory for y resolution size */
    line_addr = malloc (sizeof (__u32) * var.yres_virtual);
    addr = 0;

    /* loop to store each x resolution's start address in variable line_addr */
...
void cleansig()
{
    /* Fill the display pixel value for the give resokution */
    fillrect(
            10,      //x1
            100,     //y1
            310,    //x2
...
/* This function is used to draw cross hair */
void put_cross(int x, int y, unsigned colidx)
{
    /* draw line for the given pixel range with the given color */
...
/* this function is used to set the color index for pixel's based on number of bytes per pixel value */
...
void getpixelvalues(int x1,int y1,int x2,int y2)
{
    int tmp;
    union multiptr loc;
    ...
    for (; y1 <= y2; y1++) {
        ...
        for (tmp = x1; tmp <= x2; tmp++) {
            ...
            case 1:
            default:
                pixval[tmp-10][y1-100] =(unsigned) *loc.p8 ;
                break;

                case 2:
                pixval[tmp-10][y1-100] = (unsigned) *loc.p16 ;
                break;

                case 4:
                pixval[tmp-10][y1-100] = (unsigned)*loc.p32;
                break;
            }
...
/* get pixel value */
static inline unsigned getpixel(union multiptr loc)
{
    switch(bytes_per_pixel) {
    case 1:
    default:
...
/* Function to set color for given pixel co-ordinate */
