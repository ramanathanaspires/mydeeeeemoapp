...
const QString const_strSOCKET2APNSET                = "at+cgdcont=2,\"IP\"," ;
...
const QString const_strAVOIDCBMJUNKS                = "AT+CGREG=0";
        ...
        if ( sendCommand(const_strAT_HANDSHAKING, 300) || m_bModuleInitiated )
        {
            ...
            else
            {
                ...
                if ( sendCommand(const_strAT_SIM_STATUS,1000) )
                {
                    ...
                    //Testing
                    sendCommand(const_strM2MLOCATE,300);
                    sendCommand(const_strAVOIDCBMJUNKS,300);
...
bool CGsmModule::getNetwork(QString &r_NetworkName)
{
    ...
    if ( m_bModuleInitiated )
    {
        //        qDebug()<<"\r Network Name getnet"<<m_strNetwork<<"state"<<m_nRegStatus;
        if ( m_nRegStatus == 2)
            return false;
        if ( m_strNetwork != "" )
        {
...
bool CGsmModule::setGprsApn(const QString &r_const_strAPNNAME)
{
    ...
    if ( m_bModuleInitiated && r_const_strAPNNAME != "" )
    {
        ...
        qDebug()<<"\r apn 9"<<sendCommand(const_strAT_SET_APNNAME + const_strDOUBLE_QUOTE + r_const_strAPNNAME + const_strDOUBLE_QUOTE,1000);
        if ( m_bThreadPause )
        {
            qDebug()<<"\r apn 10";
            startThread();
...
{
    /* start the thread */
    //    qDebug()<<"\r CGSM start thread"<<isRunning()<<""<<m_bModuleInitiated;
    if ( ( ! isRunning() ) && m_bModuleInitiated )
    {
        //        qDebug()<<"\r CGSM start thread 1";
...
{
    //    qDebug()<<"\r CGSM pause thread";
    /* pause the thread */
    if ( m_bThreadRunning && isRunning() ) {
        //        qDebug()<<"\r CGSM pause thread 1";
...
void CGsmModule::run()
{
    ...
    while ( m_bThreadRunning )
    {
        ...
        gLoadFromSharedGSM(strSharedMemory);
        //        qDebug()<<"\r RUN Enter GSM SHARED"<<strSharedMemory;
        if ( const_strRUNNING == strSharedMemory || const_strGPRS_CONNECTEDCMUX ==strSharedMemory )
        {
            ...
            //            sendCommand(const_strGPSPOSITION,3000);
        }
...
{
//    qDebug()<<"\r SDK gps position";
    //    pauseThread();
    //Testing
//    qDebug()<<"\r SDK gps position 1";
    //    ////    sendCommand(const_strSOCKET2APNSET + const_strDOUBLE_QUOTE + m_strApnName + const_strDOUBLE_QUOTE,500);
    //    ////    sendCommand(const_strSOCKET2IPGET,1000);
    //    ////    sendCommand(const_strPPPIPADDRESSSECONDSOCKET,1000);
    //    ////    sendCommand(const_strCACHEDNS,1000);
    ...
    //    qDebug()<<"\r SDK gps position 2";
    ////    r_strLatitude=gm_strGPRSGPSLatitude;
    //    r_strLatitude="1";
    //    qDebug()<<"\r SDK gps position 3";
    //    r_strLongitude="2";
    ////    r_strLongitude=gm_strGPRSGPSLongitude;
    //    qDebug()<<"\r SDK gps position 4";
    //    //    if(r_strLatitude=="")
    //    //    {
    //    //        qDebug()<<"\r SDK gps position 8";
    //    //        sendCommand(const_strGPSPOSITION,1000);
    //    //        r_strLatitude=gm_strGPRSGPSLatitude;
    //    //        r_strLongitude=gm_strGPRSGPSLongitude;
    //    //    }
    ...
    //        sendCommand(const_strAVOIDCBMJUNKS,300);
    //    portInitializeCmux3(GSM_CMUX3PORTNAME);
//    qDebug()<<"\r SDK gps position 2";
    sendCommand(const_strAT_HANDSHAKING, 300);
//    qDebug()<<"\r SDK gps position 22";
    sendCommand(const_strAT_HANDSHAKING, 300);
//    qDebug()<<"\r SDK gps position 33";
    sendCommand(const_strSOCKETAVAILABLE,1000);//Testing
    qDebug()<<"\r SDK gps position 3";
    ...
    else if((m_str2SocketIP.contains("#SGACT: 2,0"))||(m_str2SocketIP.contains("#SGACT: 1")))
    {
        ...
        if(m_str2SocketIP.contains("#SGACT: 2,1"))
        {
            qDebug()<<"\r SDK gps position 44";
            sendCommand(const_strPPPIPADDRESSSECONDSOCKET,1000);
        }
        else
        {
            qDebug()<<"\r SDK gps position 444";
            sendCommand(const_strSOCKET2APNSET + const_strDOUBLE_QUOTE + m_strApnName + const_strDOUBLE_QUOTE,500);
            sendCommand(const_strSOCKETAVAILABLE,1000);
            if(m_str2SocketIP.contains("#SGACT: 2,0"))
            {
                qDebug()<<"\r SDK gps position 4444";
                sendCommand(const_strSOCKET2IPGET,1000);
                sendCommand(const_strPPPIPADDRESSSECONDSOCKET,1000);
            }
            else
            {
                qDebug()<<"\r SDK gps position 44444";
                sendCommand(const_strSOCKET2APNSET + const_strDOUBLE_QUOTE + m_strApnName + const_strDOUBLE_QUOTE,500);
                //                sendCommand(const_strSOCKETAVAILABLE,1000);
                sendCommand(const_strSOCKET2IPGET,1000);
                sendCommand(const_strPPPIPADDRESSSECONDSOCKET,1000);
...
//    qDebug()<<"\r SDK gps position 5";
    //    sendCommand(const_strSOCKET2IPGET,1000);
    //    sendCommand(const_strPPPIPADDRESSSECONDSOCKET,1000);//Testing
    if(m_str2SocketIP!="")
    {
        qDebug()<<"\r SDK gps position 6";
        sendCommand(const_strCACHEDNS,1000);
        sendCommand(const_strGPSPOSITION,7000);
        sleep(5);
        qDebug()<<"\r return after sleep";
    }
    qDebug()<<"\r SDK gps position 7";
//    r_strLatitude=gm_strGPRSGPSLatitude;
    r_strLatitude=myvariable;
    r_strLongitude=gm_strGPRSGPSLongitude;
    qDebug()<<"\r global lati"<<gm_strGPRSGPSLatitude;
    qDebug()<<"inside SDK 7 latitude"<<r_strLatitude;
    qDebug()<<"inside SDK 7 longitude"<<r_strLongitude;
...
//        r_strLatitude=gm_strGPRSGPSLatitude;
         r_strLatitude=myvariable;
        r_strLongitude=gm_strGPRSGPSLongitude;
        qDebug()<<"inside SDK 8 latitude"<<r_strLatitude;
        qDebug()<<"inside SDK 8 longitude"<<r_strLongitude;
        qDebug()<<"inside SDK gps latitude"<<gm_strGPRSGPSLatitude;
    }
...
//            r_strLatitude=gm_strGPRSGPSLatitude;
//            r_strLongitude=gm_strGPRSGPSLongitude;
//            qDebug()<<gm_strGPRSGPSLatitude;
//            qDebug()<<"latitude"<<r_strLatitude;
//            qDebug()<<"latitude"<<r_strLongitude;
//        }
...
//        r_strLongitude=gm_strGPRSGPSLongitude;
    ...
    qDebug()<<"\r latitude position"<<r_strLatitude;
    qDebug()<<"\r longitude position"<<r_strLongitude;

//    qDebug()<<"\r gm_strGPRSGPSLatitude"<<gm_strGPRSGPSLatitude<<"\n gm_strGPRSGPSLongitude"<<gm_strGPRSGPSLongitude;
    //    sleep(2);
    //    startThread();
    //    m_serialportCMUX3->close();
    return true;
}
...
//Testing
bool CGsmModule::portInitializeCmux3(const QString &r_const_strPORTNAME)
{
    //    qDebug()<<"\r gnGPRS 3..>>";
    /* initialize the port for sending and receiving data */
    m_serialportCMUX3 = new QextSerialPort();
    DEBUGMESSAGE("Gsm module port cmux name : " + r_const_strPORTNAME);
    m_serialportCMUX3->setPortName(r_const_strPORTNAME);
    m_serialportCMUX3->setDataBits(DATA_8);
    m_serialportCMUX3->setParity(PAR_NONE);
    m_serialportCMUX3->setFlowControl(FLOW_OFF);
    m_serialportCMUX3->setStopBits(STOP_1);
    m_serialportCMUX3->setBaudRate(BAUD115200);
    if(!m_serialportCMUX3->isOpen())
    {
        if ( !m_serialportCMUX3->open() ) {
            qDebug()<<"Failed to open Gsm port..!";
            return false;
        }
    }
...
int CGsmModule::readSerialData(QString &r_const_strReadBuffer)
{
    ...
    //    qDebug()<<"\r Testing byte availabdsd 123"<<nBytesAvailable;
    //Testing
    if(nBytesAvailable>=1000)
    {
        //        qDebug()<<"\r Testing byte availab";
        m_serialportSimCom->flush();
    }
    //Testing above
    if ( nBytesAvailable > 0 )
    {
        //     qDebug()<<"\r Testing byte availabdsd 12vdbhf3"<<nBytesAvailable;
...
//    qDebug()<<"\r SDK SEND";
    ...
    if ( m_serialportSimCom->isOpen() )
    {
        ...
        qDebug()<<"\r \n CGSM transmit Data"<<strData;
        if(nBytesWritten == strData.length())
        {
            qDebug()<<"\r send data GSM "<<nWait;
            //            sleep(1);//Testing
            ...
            {
//                qDebug()<<"\r send data GSM  count "<<nCount;
                if ( m_bResponse || m_bCMEError )
                {
                    if(!m_bResponse)
                    {
                        //                        qDebug()<<"\r send data GSM if ";
        ...
        if(nBytesWritten == strData.length())
        {
            ...
            for ( int nCount = 1; nCount <= nMaxCount; nCount ++ )
            {
                ...
                if ( nBytesReceived > 0 )
                {
                    ...
                    strReceivedBuffer = strReadData.left(nBytesReceived).trimmed();
                    ...
                    {
                        if(strReceivedBuffer==const_strRESPONSE_OK || strReceivedBuffer==const_strRESPOSNE_CME_ERROR || strReceivedBuffer=="ERROR")
                        {
//                            qDebug()<<"strReceivedBuffer000"<<strReceivedBuffer;

                            r_const_strRESPONSE = strReceivedBuffer;
                            m_bResponse = true;
                            break;
                        }
                        else if((strReceivedBuffer[strReceivedBuffer.length()-2]=='O') && (strReceivedBuffer[strReceivedBuffer.length()-1]=='K'))
                        {
                            r_const_strRESPONSE = strReceivedBuffer.mid(0,(strReceivedBuffer.length()-3));
//                            qDebug()<<"strReceivedBuffer111"<<strReceivedBuffer;
                            m_bResponse = true;
                            break;
                        }
                        else
                        {
                            ...
                            r_const_strRESPONSE = strReceivedBuffer;
//                            qDebug()<<"strReceivedBuffer222"<<strReceivedBuffer;
                            m_bResponse = true;
                            break;
                        }
...
//    qDebug()<<"\r \n CGSM handle msg ..>>>"<<r_const_strDATA;
    //Testing below
    if (r_const_strDATA.contains(const_strGPSPOSITIONRESPONSE))
    {
        qDebug()<<"\r checking for GPS response"<<r_const_strDATA;
        QString strGetGPS;
        ...
        //        gm_strGPRSGPSLatitude="11.0614027";
        //        gm_strGPRSGPSLongitude="76.9424017";

        qDebug()<<"\r GPS GPRS POSITION. 1234.>>"<<gm_strGPRSGPSLatitude<<gm_strGPRSGPSLongitude;
        myvariable=gm_strGPRSGPSLatitude;
        qDebug()<<"\rlatitudesssss "<<gm_strGPRSGPSLatitude;
        ...
        else if ( r_const_strDATA.contains(const_strRESPOSNE_NT_NAME) )
        {
            ...
            else
            {
                ...
                if ( r_const_strDATA.contains(",\"") )
                {
                    ...
                    //                    qDebug()<<"\r handle network if"<<m_strNetwork<<r_const_strDATA;
                }
                else
                {
                    //                    qDebug()<<"\r handle network else"<<m_strNetwork<<r_const_strDATA;
